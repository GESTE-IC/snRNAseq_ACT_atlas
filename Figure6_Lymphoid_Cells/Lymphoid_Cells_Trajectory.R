...
dataset_traj <- subset(dataset_lympho, idents = c("Plasma B cells"), invert=T)
...
GM_state <- function(cds_data){ T0_counts <- table(pData(cds_data)$State, pData(cds_data)$cell_type)[,"Unassigned T cells"]
return(as.numeric(names(T0_counts)[which(T0_counts == max(T0_counts))])) }
cds <- orderCells(cds, root_state = GM_state(cds))

saveRDS(cds, "/path/to/Monocle/results/Cds_Lymphoid_Cells.RDS")
...
diff_test_res_all <- diff_test_res_all[order(diff_test_res_all$qval),] # Suppl Table 13

saveRDS(diff_test_res_all, "/path/to/Monocle/results/DEG_byPseudotime_Lymphoid_Cells.RDS")
